...
import { checkPermission } from './auth.func'
...
router.get('/profile', (req, res) => {
  if (req.user)
    return res.status(200).json(msg.isSuccess(req.user, null))
...
router.post('/profile/update', (req, res) => {
  ...
  if (_.isEmpty(payload) || !req.user)
    return res.status(400).json(msg.badRequest(null, null))

  // check request is coming by own data
  if (req.user.permission.value < 3) {
    if (
      req.user.email != payload.email ||
      req.user.provider != payload.provider
    )
      return res
        .status(400)
        .json(msg.badRequest('User does not match.', null))
  }
...
router.post('/profile/address/update', (req, res) => {
  ...
  if (_.isEmpty(payload))
    return res
      .status(400)
      .json(msg.badRequest(null, 'payload empty.'))
  if (!user_id)
    return res
      .status(400)
      .json(msg.badRequest(null, 'user is empty.'))
