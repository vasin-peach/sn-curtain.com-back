...
router.get('/', (req, res) => {
  return res
    .status(200)
    .json(msg.isSuccess('Payment Api - Healty!', null))
})
      ...
      if (!_.isEmpty(exist)) {
        ...
        if (exist[0].discount.percent) {
          ...
          discountPrice = Math.floor(
            (exist[0].discount.percent * parseFloat(productPrice)) /
              100,
          )
        ...
        function(error, customer) {
          ...
          createOrder(payload).then(
            (response) => {
              return res
                .status(200)
                .json(msg.isSuccess(response, null))
            },
            (error) => {
              return res
                .status(200)
                .json(msg.isSuccess("can't create order.", error))
            },
          )
        },
        (error) => {
