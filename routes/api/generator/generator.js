...
router.post("/product", (req, res, next) => {
  ...
  function createProduct(num) {
    ...
    let desc = [{
      lang: "th",
      val: "Supakrit Ton ได้แชร์รูปภาพลงในกลุ่ม: ITKMITL Pre-Programming 611 ชม อันนี้เป็นเพจของOpenHouseของเรานะ เข้าไปกดไลค์ กดแชร์ให้ด้วยยย ช่วยๆกันกระจ่ายข่าว จะได้มีน้องมาเยอะๆน"
    }]
    ...
    if (Math.floor(Math.random() * 2)) {
      ...
      expired = {
        expired: true,
        expiredStart: new Date(start.getTime() + Math.random() * (end.getTime() - start.getTime())),
        expiredEnd: new Date(start.getTime() + Math.random() * (end.getTime() - start.getTime()))
      }
    }
