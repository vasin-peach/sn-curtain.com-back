...
///
router.get(
  '/get/:search/:page/:category/:type/:nature',
  async (req, res) => {
    // define filter
...
router.post('/delete', async (req, res) => {
  ...
  if (!req.body || isEmpty(req.body))
    return res
      .status(400)
      .json(msg.badRequest(null, 'bad request, `req.body` is empty.'))
...
router.post('/update', async (req, res) => {
  ...
  const brandImage =
    req.body.brand.status == 'temp' ? req.body.brand : false
  const assetsImage =
    req.body.assets.filter((x) => x.status == 'temp') || false
  ...
  if (brandImage) {
    // upload
    const uploadBrandResult = await uploadBase64(
      brandImage.src,
      name,
      req.session,
    )
  ...
  if (assetsImage) {
    // upload
    const uploadAssetsResult = await Promise.all(
      assetsImage.map((item) =>
        uploadBase64(item.src, name, req.session),
      ),
    )
