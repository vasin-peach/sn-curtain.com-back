...
router.get("/id/:code", (req, res) => {
  ...
  Discount.find({}, (err, data) => {
    ...
    var exist = discountList.filter(data => {
      let checkExpired = moment().isBetween(
        data.expired.expiredStart,
        data.expired.expiredEnd
      );

      return (
        data.code == code && (data.owner ? checkUser == data.owner : true)
...
router.get("/all", async (req, res) => {
  ...
  if (isEmpty(rawResult))
    return res.status(400).json(msg.isfail(null, "not found"));

  // decrpyt discount code
  rawResult.map((item, index) => {
    rawResult[index].code = CryptoJS.AES.decrypt(
      item.code,
      keys.ENCRYPTION_SECRET_64
    ).toString(CryptoJS.enc.Utf8);
  });
...
router.post("/delete", async (req, res) => {
  ...
  if (!req.body || isEmpty(req.body))
    return res.status(400).json(msg.badRequest(null, "payload is empty"));
...
router.post("/update", async (req, res) => {
  ...
  const decryptCode = await rawCode
    .filter(x => x._id != id)
    .map(item =>
      CryptoJS.AES.decrypt(item.code, keys.ENCRYPTION_SECRET_64).toString(
        CryptoJS.enc.Utf8
      )
    );

  // * Check if update code is same as code in database
  if (decryptCode.includes(payload.code))
    return res.status(400).json(msg.badRequest(null, "code is exist"));

  // * Encrypt update code
  payload.code = CryptoJS.AES.encrypt(
    payload.code,
    keys.ENCRYPTION_SECRET_64
  ).toString();

  if (!payload._id) payload._id = mongoose.Types.ObjectId();
  if (payload.owner == "" || payload.owner == " " || payload.owner == null)
    payload.owner = null;
  ...
  Discount.findOneAndUpdate(
    ...
    (err, data) => {
      if (err) return res.status(400).json(msg.isfail(data, err));
        ...
        data.code = CryptoJS.AES.decrypt(
          data.code,
          keys.ENCRYPTION_SECRET_64
        ).toString(CryptoJS.enc.Utf8);
        return res.status(200).json(msg.isSuccess(data, err));
...
router.post("/create", async (req, res) => {
  ...
  if (_.isEmpty(req.body.payload))
    return res.status(400).json(msg.badRequest("bad param, payload is empty."));
  if (!req.body.payload.code)
    return res
      .status(400)
      .json(msg.badRequest("bad param, payload code is empty."));
