...
router.get('/id/:code', (req, res) => {
  ...
  Discount.find({}, (err, data) => {
    ...
    var exist = discountList.filter((data) => {
      ...
      return (
        data.code == code &&
        (data.owner ? checkUser == data.owner : true)
    ...
    // response
    if (_.isEmpty(exist))
      return res.status(404).json(msg.isEmpty(null, null))
    else {
      return res.json(
        msg.isSuccess(
          {
...
router.post('/delete', async (req, res) => {
  ...
  if (!req.body || isEmpty(req.body))
    return res
      .status(400)
      .json(msg.badRequest(null, 'payload is empty'))
...
router.post('/create', async (req, res) => {
  ...
  if (_.isEmpty(req.body.payload))
    return res
      .status(400)
      .json(msg.badRequest('bad param, payload is empty.'))
