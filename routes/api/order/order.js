...
router.get('/:id', (req, res) => {
  ...
  Order.findOne(
    ...
    (err, data) => {
      ...
      if (err) return res.status(400).json(msg.isfail(data, err))
      if (isEmpty(data))
        return res.status(400).json(msg.isEmpty(data, err))
...
router.post('/', async (req, res) => {
  ...
  if (isEmpty(payload) || !payload.product)
    return res
      .status(400)
      .json(msg.isEmpty(null, 'Payload is empty.'))
  ...
  description = product.reduce((sum, item) => {
    return `${sum + item.data.name} (${item.buyOption}฿)[${
      item.amount
    }].`
  }, email)
    ...
    if (!isEmpty(discountExist)) {
      ...
      if (discountExist[0].discount.percent) {
        ...
        discountPrice = Math.floor(
          (discountExist[0].discount.percent *
  ...
  createOrder(obj).then(
    ...
    (error) => {
      return res
        .status(400)
        .json(msg.isfail("can't create order.", error))
    },
  )
})
...
router.post('/delete', async (req, res) => {
  ...
  if (!delete_id || !delete_user) {
    ...
    if (authPermissionLevel[0])
      ...
      return res
        .status(401)
        .json(msg.unAccess('invalid access level'))
  }
  ...
  await Order.findOne(
    ...
    (error, result) => {
      if (error) return res.status(400).json(msg.isfail(null, error))
        ...
        return res
          .status(200)
          .json(msg.isEmpty(null, 'result is empty.'))
      else if (!result.user_id)
        return res
          .status(404)
          .json(msg.isEmpty(null, 'order not found'))
      else if (result.user_id != delete_user)
        return res
          .status(400)
